...
#include "jjg.h"
#include "dxt_img.h"
#include "shp23dtile.cpp" // By JIAO Jingguo 2023.7.2
using namespace std;
...
Polygon_Mesh convert_polygon(OGRPolygon* polyon, double center_x, double center_y, double height)
{
	//double bottom = 0.0;
	Polygon_Mesh mesh;
...
bool shp23dtile(const char* filename, int layer_id, const char* dest, const char* height)
{
...
tinygltf::Material make_color_material(double r, double g, double b) {
	...
	material.values["baseColorFactor"] = baseColorFactor;
	tinygltf::Parameter metallicFactor;
	metallicFactor.number_value = 0.3;
	material.values["metallicFactor"] = metallicFactor;
	tinygltf::Parameter roughnessFactor;
	roughnessFactor.number_value = 0.7;
	material.values["roughnessFactor"] = roughnessFactor;
	return material;
}
...
int OSGB_To_3dtiles_JJG(std::string inputFolder, std::string outputDir) // By JIAO Jingguo 2023.7.2 整理OSGB转3dtiles函数 经测试可运行
{
	...
	*/
	...
	std::cout << "osgb转3dtiles结束 By JIAO Jingguo 2023.7.2" << std::endl;
	return 1;
}
